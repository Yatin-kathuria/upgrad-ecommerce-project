...
const Login = ({ userInfo, setUserInfo }) => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      // Assuming username and password are state variables or gathered from form fields
      ...
      if (!res.ok) {
        if (res.status === 401) {
          ...
          throw new Error(`There was a problem with the Fetch operation: ${res.status}`);
        }
      }
